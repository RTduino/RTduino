...
#else
  //1 平台问题已经解决
  //2 字符串mock函数有问题还没有解决 实际Serial输出正确 但是mock输出不对
  // -1FFFFFFFF vs FFFFFFFF
  WHEN("HEX") { mock.print(val, HEX); rt_kprintf("%s\n", mock._str.c_str()); Serial.println(val, HEX); REQUIRE(mock._str  == "FFFFFFFF"); }
  // -1FFFFFFFF37777777777 vs 37777777777
  WHEN("OCT") { mock.print(val, OCT); rt_kprintf("%s\n", mock._str.c_str()); Serial.println(val, OCT); REQUIRE(mock._str  == "37777777777"); }
  // -1FFFFFFFF3777777777711111111111111111111111111111111 vs 11111111111111111111111111111111
  WHEN("BIN") { mock.print(val, BIN); rt_kprintf("%s\n", mock._str.c_str()); Serial.println(val, BIN); REQUIRE(mock._str  == "11111111111111111111111111111111"); }
