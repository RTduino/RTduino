...
boolean SerialFirmata::handlePinMode(byte pin, int mode)
{
  ...
  if (mode == FIRMATA_PIN_MODE_SERIAL) {
    Firmata.setPinMode(pin, FIRMATA_PIN_MODE_SERIAL);
    return true;
  }
  return false;
}
...
void SerialFirmata::handleCapability(byte pin)
{
  if (IS_PIN_SERIAL(pin)) {
    Firmata.write(FIRMATA_PIN_MODE_SERIAL);
          ...
          if (portId < 8) {
            ...
            if (serialPort != NULL) {
              ...
              if (pins.rx != 0 && pins.tx != 0) {
                Firmata.setPinMode(pins.rx, FIRMATA_PIN_MODE_SERIAL);
                Firmata.setPinMode(pins.tx, FIRMATA_PIN_MODE_SERIAL);
